...
export type GeneratedMode = "check" | "write" | "invalid";

/** Classifies the script's direct CLI mode without weakening its default checking behavior. */
export function generatedMode(args: readonly string[]): GeneratedMode {
  if (args.length === 0) {
    return "check";
  }

  return args.length === 1 && args[0] === "--write-barrel" ? "write" : "invalid";
}

/** Renders the type-only barrel for direct ts-rs model files in stable lexical order. */
export function renderGeneratedBarrel(entries: readonly string[]): string {
  ...
  return (
    GENERATED_HEADER +
    models.map((model) => `export type { ${model} } from "./${model}";\n`).join("")
  );
}

/** Writes the generated core-client type barrel beside its sibling ts-rs model files. */
export function writeGeneratedBarrel(root: string): void {
  const generated = join(root, EXPORT_DIR);
  const entries = readdirSync(generated);
  writeFileSync(join(generated, "index.ts"), renderGeneratedBarrel(entries));
}
...
if (invokedDirectly) {
  ...
  const mode = generatedMode(process.argv.slice(2));

  if (mode === "invalid") {
    process.stderr.write("expected no arguments or --write-barrel\n");
    process.exit(1);
  }
