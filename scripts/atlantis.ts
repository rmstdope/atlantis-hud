...
async function runRefresh(io: Io, json: boolean): Promise<number> {
  ...
  // Trial run first, against real OS temp files rather than the fixtures - the scraper resolves a
  // relative path against the repository root, so these are absolute deliberately. Nothing under the committed tree is touched
