...
describe("renderGeneratedBarrel", () => {
  it("sorts generated model names, excludes its own index, and keeps the generated-file header", () => {
    expect(renderGeneratedBarrel(["Zebra.ts", "index.ts", "Apple.ts"])).toBe(
...
describe("writeGeneratedBarrel", () => {
  ...
  it("writes the generated barrel beside the generated models", () => {
    const root = mkdtempSync(join(tmpdir(), "write-generated-barrel-"));
    fixtures.push(root);
    const generated = join(root, "packages/core-client/src/generated");
    mkdirSync(generated, { recursive: true });
    writeFileSync(join(generated, "Zebra.ts"), "");
    writeFileSync(join(generated, "Apple.ts"), "");

    writeGeneratedBarrel(root);

    expect(readFileSync(join(generated, "index.ts"), "utf8")).toBe(
      renderGeneratedBarrel(["Apple.ts", "Zebra.ts"])
    );
  });
});

describe("generatedMode", () => {
  it("accepts only the explicit write mode and preserves checking by default", () => {
    expect(generatedMode([])).toBe("check");
    expect(generatedMode(["--write-barrel"])).toBe("write");
    expect(generatedMode(["--unexpected"])).toBe("invalid");
  });
});
...
describe("compareTrees", () => {
  ...
  it("reports a stale generated barrel as a divergence", () => {
    const barrel = "packages/core-client/src/generated/index.ts";

    expect(compareTrees(new Map([[barrel, "fresh"]]), new Map([[barrel, "stale"]]))).toEqual([
      { path: barrel, reason: "differs" }
    ]);
    expect(compareTrees(new Map([[barrel, "fresh"]]), new Map())).toEqual([
      { path: barrel, reason: "missing" }
    ]);
    expect(compareTrees(new Map(), new Map([[barrel, "stale"]]))).toEqual([
      { path: barrel, reason: "unexpected" }
    ]);
  });
