...
const RULES_HTML = readFileSync(
  fileURLToPath(
    new URL(
      "../../../tests/fixtures/ruleset/neworigins-rules.html",
      import.meta.url,
    ),
  ),
  "utf8",
...
const ARCANUM_RULES_HTML = readFileSync(
  fileURLToPath(
    new URL(
      "../../../tests/fixtures/ruleset/newage-arcanum-rules.html",
      import.meta.url,
    ),
  ),
  "utf8",
...
const TRIDENT_RULES_HTML = readFileSync(
  fileURLToPath(
    new URL(
      "../../../tests/fixtures/ruleset/newage-trident-rules.html",
      import.meta.url,
    ),
  ),
  "utf8",
...
describe("parseMovementRules", () => {
  ...
  it("takes the affected modes from the page rather than assuming them", () => {
    ...
    expect(parseMovementRules(reworded).terrainCosts.premiumFor).toEqual([
      "walk",
    ]);
  });
  ...
  it("reads the lake addition to the water rule", () => {
    ...
    expect(trident.provenance.ocean).toContain(
      "Lakes count as water for this purpose",
    );

    const origins = parseMovementRules(RULES_HTML);

    expect(origins.ocean.alsoWater).toEqual([]);
    expect(origins.provenance.ocean).not.toContain(
      "count as water for this purpose",
    );
  });
  ...
  it("reads New Age's swimming rule", () => {
    ...
    expect(trident.swimming).toEqual({
      unrestricted: ["lake"],
  ...
  it("refuses a swimming rule restricting a terrain the water rule does not name", () => {
    // The fixture is the page verbatim and wraps mid-sentence, so the two halves of the
    // terrain name are reworded separately.
  ...
  it("records the sentence every value came from", () => {
    ...
    expect(rules.provenance.movementPoints).toContain(
      "Walking units have two movement points",
    );
    expect(rules.provenance.terrainCosts).toContain("take two movement points");
    expect(rules.provenance.road).toContain(
      "half cost to a minimum of 1 movement point",
    );
    expect(rules.provenance.ocean).toContain(
      "must end their movement on land or else drown",
    );
  });
  ...
  it("records the sentences the sailing rule came from", () => {
    ...
    expect(rules.provenance.sailing).toContain(
      "one movement point. A coastal region",
    );
  });
  ...
  it("reads a tiered terrain sentence, pricing a volcano above a forest", () => {
    ...
    expect(rules.terrainCosts.premiumFor).toEqual(["ride", "walk"]);
    expect(rules.provenance.terrainCosts).toContain(
      "4 movement points for volcano",
    );
  ...
  it("fails loudly when a tier clause is not a cost and a list", () => {
    ...
    expect(() => parseMovementRules(reworded)).toThrowError(RulesetScrapeError);
    expect(() => parseMovementRules(reworded)).toThrowError(
  ...
  it("fails loudly when one terrain is priced in two tiers at once", () => {
    ...
    expect(() => parseMovementRules(reworded)).toThrowError(RulesetScrapeError);
    expect(() => parseMovementRules(reworded)).toThrowError(
      /forest is priced at both 2 and 4/,
    );
  });
...
describe("parseFoodMaintenance", () => {
  ...
  it("refuses a rules page that never prices food", () => {
    expect(() =>
      parseFoodMaintenance(
        "<html><body>a page about something else</body></html>",
      ),
...
describe("parseWeatherGap", () => {
  ...
  it("refuses a rules page that says nothing about weather", () => {
    expect(() =>
      parseWeatherGap("<html><body>no weather here</body></html>"),
    ).toThrowError(RulesetScrapeError);
    expect(() =>
      parseWeatherGap("<html><body>no weather here</body></html>"),
    ).toThrowError(/weatherRule/);
  });
});
...
describe("parseRegionResources", () => {
  ...
  it("reads the region resources table", () => {
    ...
    expect(resources.swamp).toEqual([
      "wood",
      "floater hide",
      "herb",
      "mushroom",
...
describe("anchoredTableRows", () => {
  ...
  it("answers nothing for an anchor with no table after it", () => {
    const html =
      '<html><body><a name="lonely"></a><p>prose only</p></body></html>';
